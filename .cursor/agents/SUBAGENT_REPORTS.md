...
### 2026-06-15 — Decky Plugin Studio v0.2.0 gap closure

**Scope:** Dynamic RPC allowlist, deploy parity, preview test kit, permission simulator, UI shims, IPC reliability, agent pack expansion.

**Subagents:** security-auditor / red-team / blue-team — **deferred** (ship window; triage at next security pass). **master-debugger** — N/A until on-device screenshot validation.

**Tests:** `mcp-server` rpcDiscover tests pass; `example-plugin` vitest 5/5 pass; `pnpm run build` green.

**VSIX:** `pnpm run package:vsix` may fail on Windows EPERM when rebundling `extension/resources` — retry after closing handles or run `deploy:cursor:build-only`.
