...
- **deck.holdAwake** `{ ttlMinutes?: 30, note? }` / **deck.restorePowerSettings** `{}` — **broken; do not use.** Built to turn off the screen-blank and suspend timeouts for a test run and put the previous values back. Measured on hardware 2026-09-08: it holds nothing (`xset` DPMS is absent on this Deck's Xwayland, and Game Mode sleep is gamescope's, not logind's) and its restore can write back a setting that was never there, while both report success. Set Steam's own sleep timer to Never before a long unattended run instead. Replacement is a research spike in [ROADMAP.md](ROADMAP.md#hold-the-deck-awake-for-a-test-run-then-put-the-setting-back). **The battery warning below still applies to whatever replaces it.**
