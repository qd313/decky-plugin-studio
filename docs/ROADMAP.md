...
- **Still device-unverified.** The desk proof is a fake CDP server and a simulated press. The real proof is the bridge board's Deck-side lead unplugged: every one of these tools must then report `wire-sent`, and that is the outstanding manual step below.
