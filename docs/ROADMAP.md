...
| **`deck_deploy` guesses the plugin directory and gets the case wrong** | ★ | Open — found 2026-08-27 | Failed with a bare `Error: Command failed: scp -r ... deck@…:~/homebrew/plugins/bonsai/dist` against a plugin installed at `~/homebrew/plugins/**bonsAI**/`. Two problems behind one unhelpful message: the target name is lower-cased rather than read from `plugin.json`'s `name`, and the installed directory is root-owned so a plain `scp` as `deck` could not have written it even with the right case. The error surfaces neither fact. Read the name from the manifest, and when the destination is not writable say so instead of echoing the failed command. |
